 ...
 * License: MIT
 *
 * This Chrome Extension version adapted from the original Userscript.
 */

(function () {
    // Chrome Storage API wrapper to replace GM_getValue/GM_setValue
    const Storage = {
    ...
    class ExclusionManager {
        static async updateExclusionList(itemId, isAdding) {
            const currentValue = await Storage.get("exclude_item_ids", "");
            if (isAdding) {
                await Storage.set("exclude_item_ids", currentValue + ` ${itemId}`);
            } else {
                await Storage.set(
                    "exclude_item_ids",
        ...
        static async setupExclusionButtons() {
            ...
            for (let index = 0; index < itemElements.length; index++) {
                ...
                const button = await this.createExcludeButton(item, index);
        ...
        static async createExcludeButton(item, index) {
            const button = document.createElement("button");
            const itemId = item.href.match(/\d+$/g)[0];
            const excludeList = await Storage.get("exclude_item_ids", "");
            const isExcluded = String(excludeList).includes(itemId);
            ...
            document.addEventListener("keydown", async (event) => {
                ...
                if (
                    ...
                    if (confirm("除外設定を全てリセットしますか?")) {
                        await Storage.set("exclude_item_ids", "");
        ...
        static async handleExcludeClick(event, index, item) {
            const button = document.querySelector(`#excludeButton${index}`);
            const itemId = item.href.match(/\d+$/g)[0];

            if (event.ctrlKey) {
                await this.toggleAllExclusions();
            } else {
                await this.toggleSingleExclusion(button, itemId);
            }
        }

        static unifiedState = true;

        static async toggleAllExclusions() {
            ...
            for (const button of allButtons) {
                ...
                await this.updateExclusionList(itemId, newState);
            }
        }

        static async toggleSingleExclusion(button, itemId) {
            const isExcluded = button.className === "ex_false";
            button.style.background = isExcluded ? "#e1362e" : "#808080";
            button.textContent = isExcluded ? "除外解除" : "除外する";
            button.className = isExcluded ? "ex_true" : "ex_false";
            await this.updateExclusionList(itemId, isExcluded);
        }
    }

    // Item Management
    class ItemManager {
        static async collectItemInfo(itemElement) {
            ...
            const excludeList = await Storage.get("exclude_item_ids", "");
            if (String(excludeList).indexOf(itemId) !== -1) {
                return "exclude";
            }
        ...
        static async fetchItemPrice(orderId) {
            try {
                ...
                return {
                    item_price: matched ? Number(matched[1].replace(/,/g, "")) : undefined,
                    is_gift: isGift,
                    finalUrl: response.url // リダイレクト後の最終URLを記録
                };
            } catch (error) {
    ...
    async function main() {
        ...
        const itemListPromises = itemListElements.map(ItemManager.collectItemInfo);
        let itemList = await Promise.all(itemListPromises);
        console.log("Collected item list:", itemList);

        // 既存の累計を取得
        const existingTotal = parseFloat(url.searchParams.get("total")) || 0;
        const existingGiftTotal = parseFloat(url.searchParams.get("gift_total")) || 0;

        if (itemList.every((v) => v === "exclude") && itemList.length !== 0) {
            URLManager.processNextPage(window.location.href);
            return { totalPrice: existingTotal, giftTotal: existingGiftTotal };
        ...
        for (const itemInfo of itemList) {
            try {
                ...
                if (itemPrice.finalUrl && processedOrderUrls.has(itemPrice.finalUrl)) {
                    console.log(`[DEBUG] ⏭️ スキップ(既に処理済み): ${itemPrice.finalUrl}`);
                    completedItems++;
                    progress.update(completedItems, totalItems);
                    continue;
                }
                ...
                // 処理済みとして記録
                if (itemPrice.finalUrl) {
                    processedOrderUrls.add(itemPrice.finalUrl);
                }
